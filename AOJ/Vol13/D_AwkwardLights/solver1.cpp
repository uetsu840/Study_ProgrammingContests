...
public:
    ...
    bool solve1DEquation(void) {
        for (SQWORD sqPivotIdx = 0; sqPivotIdx < nRows; sqPivotIdx++) {
            bool bPivot = true;
            if (0 == getValue(sqPivotIdx, sqPivotIdx)) {
                /* search non zero pivot  ->  swap rows */
                SQWORD sqSearchRow = sqPivotIdx + 1;
                for (; sqSearchRow < nRows; sqSearchRow++) {
                    if (0 != getValue(sqSearchRow, sqPivotIdx)) {
                        break;
                    }
                }
                if (sqSearchRow == nRows) {
                    ...
                    swapRows(sqPivotIdx, sqSearchRow);
                }
            }
            /* pivot */
            if (bPivot) {
                for (SQWORD sqTargetRow = 0; sqTargetRow < nRows; sqTargetRow++) {
                    if (sqTargetRow != sqPivotIdx) {
                        if (1 == getValue(sqTargetRow, sqPivotIdx)) {
                            diffRows(sqTargetRow, sqPivotIdx);
                        }
                    }
                }
            }
...
bool solve(void)
{
    ...
    /* Configure A */
    configureCoeffs(matEq, sqInput_M, sqInput_N, sqInput_D);

    /* input b */
    for (SQWORD sqRowIdx = 0; sqRowIdx < sqInput_M; sqRowIdx++) {
        for (SQWORD sqColIdx = 0; sqColIdx < sqInput_N; sqColIdx++) {
            SQWORD sqInput_S = inputSQWORD();
            SQWORD sqEqIdx = getEqCoeffIdx(sqRowIdx, sqColIdx, sqInput_M, sqInput_N);
    ...
    matEq.print();
    /* solve */
    matEq.solve1DEquation();

    bool bAnsExist = true;
    for (SQWORD sqRowIdx = 0; sqRowIdx < sqDim; sqRowIdx++) {
        for (SQWORD sqColIdx = 0; sqColIdx < sqDim; sqColIdx++) {
            if (sqRowIdx == sqColIdx) {
                if (0 == (matEq.getValue(sqRowIdx, sqColIdx))) {
                    if (0 < matEq.getValue(sqRowIdx, sqDim))  {
                        bAnsExist = false;
                        break;
                    }
                }
