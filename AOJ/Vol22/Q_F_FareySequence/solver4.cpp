...
static void getPrimes(vector<SDWORD> &vlPrimes, SDWORD lMax)
{
    ...
    /* 素数のリストを作る */
    vector<bool> vbIsPrime(lMax, true);
...
int main(void)
{
    ...
    static set<pair<SQWORD, SQWORD>> vMutuallyPrimeNums;
    ...
    /* オイラー関数を計算する */
    vMutuallyPrimeNums.insert(make_pair(1, 1));
    for (auto prime: vlPrimes) {
        vpairAddMP.clear();

        /* 素数 */
        auto it = vMutuallyPrimeNums.begin();
        while (it != vMutuallyPrimeNums.end()) {
            ...
            if (MAX_N / prime < it->first) {
                it = vMutuallyPrimeNums.erase(it);
            } else {
                ++it;
            }
        }
        for (auto addMP: vpairAddMP) {
            vMutuallyPrimeNums.insert(addMP);
        }
    }
