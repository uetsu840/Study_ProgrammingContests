...
#else
			for (int k = 0; k < sqSz; k++) {
                if (vecBalls[k].dEnd < vecBalls[j].dBegin) {
        			best = max(best, s_aadDpTbl[i-1][k] + vecBalls[j].dEnd - vecBalls[j].dBegin);
                } else {
          			best = max(best, s_aadDpTbl[i-1][k] + vecBalls[j].dEnd - vecBalls[k].dEnd);
                }
            }
            s_aadDpTbl[i][j] = best;
#endif
