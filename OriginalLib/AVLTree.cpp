...
class AVLTree {
    ...
    /* ノードの追加の追加 */
    int addNode(TREE_NODE **node, int data) {
     ...
     *  検索
     */
    bool findNode(TREE_NODE **node, int findValue) {
        Trace("AVLTree findNode node[%08x]\n", *node);
        if (*node == nullptr) {
            Trace("AVLTree findNode [%lld] nullptr\n", findValue);
            return false;
        }
        Trace("AVLTree findNode value[%lld]\n", (*node)->data);
        if ((*node)->data == findValue) {
            Trace("AVLTree findNode [%lld] found\n", findValue);
            return true;
        } 
        if (findValue < (*node)->data) {
            Trace("AVLTree findNode > R (%08x)\n", (*node)->child[0]);
            return findNode(&((*node)->child[0]), findValue);
        } else {
            Trace("AVLTree findNode > L (%08x)\n", (*node)->child[1]);
...
int main()
{
    ...
    tree.insert(100);
    tree.insert(101);
    for (int value = 99; 0 < value; value--) {
        tree.insert(value);
    }
    tree.insert(200);
    tree.debugPrint();
