...
//                printf("   %lld--> %lld\n", sqFrom, sqTo);
    ...
    vector<bool> vbUsed(vvstCompEdge.size(), false);
    vector<SQWORD> vsqSccTopological;
    vsqSccTopological.emplace_back(0);
    for (SQWORD sqNode = 1; sqNode <= sqCompNodeNum; sqNode++) {
        if (!vbUsed[sqNode]) {
//            printf("%lld\n", sqNode);
...
//                        printf("ts: %lld \n", e.sqTo);
    ...
    vector<vector<EDGE_ST>> vvstCompEdgeTopo(vvstCompEdge.size());
    vector<vector<SQWORD>> vvCompNodesTopo(vvCompNodes.size());

    for (SQWORD sqIdx = 0; sqIdx < vsqSccTopological.size(); sqIdx++) {
        vvstCompEdgeTopo[sqIdx] = vvstCompEdge[vsqSccTopological[sqIdx]];
        vvCompNodesTopo[sqIdx]  = vvCompNodes[vsqSccTopological[sqIdx]];
    }

    SQWORD sqCmpN = vvCompNodesTopo.size();
    vector<SQWORD> vParentSum(sqCmpN, 0);
    for (SQWORD sqIdx = 1; sqIdx < sqCmpN; sqIdx++) {
        vParentSum[sqIdx] += vvCompNodesTopo[sqIdx].size();
    }
    for (SQWORD sqIdx = 1; sqIdx < sqCmpN; sqIdx++) {
        queue<SQWORD> queCurNode;
        SQWORD sqFrontSize = vvCompNodesTopo[sqIdx].size();
        vector<SQWORD> vbIsVisited(sqCmpN + 1, false);
        queCurNode.push(sqIdx);
        while (!queCurNode.empty()) {
            SQWORD sqCurNode = queCurNode.front();
            queCurNode.pop();
            vbIsVisited[sqCurNode] = true;
            for (auto e: vvstCompEdgeTopo[sqCurNode]) {
                if (!vbIsVisited[e.sqTo]) {
                    vParentSum[e.sqTo] += sqFrontSize;
                    queCurNode.push(e.sqTo);
                }
            }
        }
    }
    ...
    DOUBLE dfAns = 0.0;
    for (SQWORD sqIdx = 1; sqIdx < sqCmpN; sqIdx++) {
//        printf("%lld %lld %lld\n", sqIdx, vParentSum[sqIdx], vvCompNodesTopo[sqIdx].size());
        dfAns += (DOUBLE)(vvCompNodesTopo[sqIdx].size()) / (DOUBLE)vParentSum[sqIdx];
    }
    printf("%0.11f\n", dfAns);
