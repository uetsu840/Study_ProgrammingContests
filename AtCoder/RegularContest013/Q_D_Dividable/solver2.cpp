...
private:
    SQWORD maxFlowDfs(SDWORD lV, SDWORD lT, SQWORD sqF)
    {
        if (lV == lT) {
            return sqF;
        }
        ...
        for (SDWORD lIdx = 0; lIdx < avGraph[lV].size(); lIdx++) {
            ST_EDGE &stE = avGraph[lV][lIdx];
            if ((!abIsUsed[stE.lTo]) && (0 < stE.sqCap)) {
                SDWORD lD = maxFlowDfs(stE.lTo, lT, min(sqF, stE.sqCap));
                if (lD > 0) {
                    stE.sqCap -= lD;
                    avGraph[stE.lTo][stE.lRev].sqCap += lD;
                    return lD;
                }
            }
        }
...
public:
    SQWORD MaxFlow(SDWORD lStart, SDWORD lTarget)
    {
        SQWORD sqFlow = 0LL;
        for (;;) {
            memset(abIsUsed, 0, sizeof(abIsUsed));
            SQWORD sqDiffFlow = maxFlowDfs(lStart, lTarget, MAX_SQWORD); 
            if (0 == sqDiffFlow) {
                return sqFlow;
            }
            sqFlow += sqDiffFlow;
