...
int main()
{
    ...
    /* 装置番号をインデックスで置き換える */
    map<QWORD, DWORD> change_map;
    DWORD dwMachineIdx = 0;
    for (DWORD dwChgIdx = 0; dwChgIdx < dwInput_M; dwChgIdx++) {
        QWORD qwMachineNo = s_astChanges[dwChgIdx].qwMachineNo;
        if (0 == change_map.count(qwMachineNo)) {
            change_map.insert(make_pair(qwMachineNo, dwMachineIdx));
            vMachineNoList.push_back(qwMachineNo);
            dwMachineIdx++;
        }
    }
    sort(vMachineNoList.begin(), vMachineNoList.end());
    for (DWORD dwChgIdx = 0; dwChgIdx < dwInput_M; dwChgIdx++) {
        QWORD qwMachineNo = s_astChanges[dwChgIdx].qwMachineNo;
        DWORD dwMachineIdx = upper_bound(vMachineNoList.begin(), vMachineNoList.end(), qwMachineNo) - vMachineNoList.begin();
        s_astChanges[dwChgIdx].qwMachineNo = dwMachineIdx;
    }
