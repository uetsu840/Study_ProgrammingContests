...
int main()
{
    ...
    {
        typedef std::pair<SQWORD,SQWORD> ipair;
        static ipair aqwCityId[CITY_INFO_ARY_SIZE];
        for (DWORD dwCityIdx = 1; dwCityIdx <= dwInput_N; dwCityIdx++) {
            aqwCityId[0].first = 0;
            aqwCityId[0].second = 0;
            DWORD dwRootRoad = ufGetRoot(&stUfRoad, dwCityIdx);
            DWORD dwRootRail = ufGetRoot(&stUfRail, dwCityIdx);
            aqwCityId[dwCityIdx].first = ((QWORD)dwRootRoad << 32) + ((QWORD)dwRootRail);
            aqwCityId[dwCityIdx].second = dwCityIdx;
        }
        sort(&(aqwCityId[1]), &(aqwCityId[dwInput_N + 1]));
        DWORD dwConnectCount = 1;
        static DWORD adwConnectCounts[CITY_INFO_ARY_SIZE];
        for (DWORD dwCityIdx = 1; dwCityIdx <= dwInput_N; dwCityIdx++) {
//            printf("%d\n", aqwCityId[dwCityIdx].second);
            if ((dwCityIdx == dwInput_N)) {
                for (DWORD dwConnectIdx = dwCityIdx - (dwConnectCount - 1); dwConnectIdx <= dwCityIdx; dwConnectIdx++) {
                    adwConnectCounts[aqwCityId[dwConnectIdx].second] = dwConnectCount;
                }
            }
            if ((aqwCityId[dwCityIdx].first != aqwCityId[dwCityIdx+1].first)) {
                for (DWORD dwConnectIdx = dwCityIdx - (dwConnectCount - 1); dwConnectIdx <= dwCityIdx; dwConnectIdx++) {
                    adwConnectCounts[aqwCityId[dwConnectIdx].second] = dwConnectCount;
                }
                dwConnectCount = 1;
        ...
        for (DWORD dwIdxJ = 1; dwIdxJ <= dwInput_N; dwIdxJ++) {
            printf("%d", adwConnectCounts[dwIdxJ]);
//            printf("%d\n", dwIdxJ);
            if (dwIdxJ != dwInput_N) {
                printf(" ");
            }
