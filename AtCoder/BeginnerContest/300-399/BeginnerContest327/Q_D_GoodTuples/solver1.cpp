...
int main(void)
{
    ...
    for (SQWORD sqIdx = 0; sqIdx < sqM; sqIdx++) {
        ...
        sqS--;
        vsqS.emplace_back(sqS);
    }
    for (SQWORD sqIdx = 0; sqIdx < sqM; sqIdx++) {
        SQWORD sqT = inputSQWORD();
        sqT--;
        vsqT.emplace_back(sqT);
    }

    vector<vector<SQWORD>> vvsqEdge(sqN);
    for (SQWORD sqIdx = 0; sqIdx < sqM; sqIdx++) {
        SQWORD sqS = vsqS[sqIdx];
        SQWORD sqT = vsqT[sqIdx];
        vvsqEdge[sqS].emplace_back(sqT);
        vvsqEdge[sqT].emplace_back(sqS);
    }

    vector<SQWORD> vsqColors(sqN, -1);
    for (SQWORD sqFrom = 0; sqFrom < sqN; sqFrom++) {
        if (vsqColors[sqFrom] == -1) {
            vsqColors[sqFrom] = 0;
            if (!dfs(sqFrom, vsqColors, 0, vvsqEdge)) {
                printf("No\n");
                return 0;
            }
        }
