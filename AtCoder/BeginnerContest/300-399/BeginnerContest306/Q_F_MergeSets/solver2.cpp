...
int main(void)
{
    ...
    vector<vector<SQWORD>> vvsqA(sqN);
    for (SQWORD sqIdxI = 0; sqIdxI < sqN; sqIdxI++) {
        for (SQWORD sqIdxJ = 0; sqIdxJ < sqM; sqIdxJ++) {
            SQWORD sqA = inputSQWORD();
            vvsqA[sqIdxI].emplace_back(sqA);
        }
        sort(vvsqA[sqIdxI].begin(), vvsqA[sqIdxI].end());
    }

    vector<SQWORD> vsqVals;
    for (SQWORD sqIdxI = 0; sqIdxI < sqN; sqIdxI++) {
        for (SQWORD sqIdxJ = 0; sqIdxJ < sqM; sqIdxJ++) {
            SQWORD sqA = vvsqA[sqIdxI][sqIdxJ];
            vsqVals.emplace_back(sqA);
        }
    }
    sort(vsqVals.begin(), vsqVals.end());
        
    map<SQWORD, SQWORD> mapVals;
    for (SQWORD sqIdx = 0; sqIdx < vsqVals.size(); sqIdx++) {
        mapVals[vsqVals[sqIdx]] = sqIdx;
    }

//    printf("%lld\n", mapVals.size());
    BinaryIndexedTree<SQWORD> bit(mapVals.size());


    for (SQWORD sqIdx = 0; sqIdx < mapVals.size(); sqIdx++) {
        bit.Add(sqIdx + 1, 1);
    }
    ...
    for (SQWORD sqIdxI = 0; sqIdxI < sqN; sqIdxI++) {
        for (SQWORD sqIdxJ = 0; sqIdxJ < sqM; sqIdxJ++) {
            SQWORD sqValIdx = mapVals[vvsqA[sqIdxI][sqIdxJ]];
            SQWORD sqOrder =  bit.Sum(sqValIdx);
//            printf(" [%lld %lld]order : %lld\n", sqIdxI, sqIdxJ, sqOrder);
            sqAns += (sqOrder - sqIdxJ) + (sqIdxJ + 1) * (sqN - 1 - sqIdxI);
        }

        for (auto a: vvsqA[sqIdxI]) {
            SQWORD sqValIdx = mapVals[a];
            bit.Add(sqValIdx + 1, -1);
        }
    }
