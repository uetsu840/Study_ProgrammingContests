...
{
    sqCount++;
    vbVisited[sqStart] = true;
    for (SQWORD e:  vvsqEdge[sqStart]) {
        if (!vbVisited[e] && (e != sqFrom)) {
            dfs(sqStart, e, vvsqEdge, vbVisited, sqCount);
        }
    }
}
...
int main(void)
{
    ...
    vector<SQWORD> vsqSaikyo;
    for (SQWORD sqStart = 0; sqStart < sqN; sqStart++) {
        SQWORD sqCount = 0;
        vector<bool> vbVisited(sqN);
        dfs(-1, sqStart, vvsqEdge, vbVisited, sqCount);

        if (sqCount == sqN) {
            vsqSaikyo.emplace_back(sqStart);
        }
    }
    if (vsqSaikyo.size() == 1) {
        printf("%lld\n", vsqSaikyo[0] + 1);
