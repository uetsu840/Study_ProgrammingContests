...
bool operator< (const SENSOR &a, const SENSOR &b) {
    if (a.sqStart == b.sqStart) {
        return a.sqEnd < b.sqEnd;
    }
    return a.sqStart < b.sqStart;
}


auto greaterEnd = [] (const SENSOR &a, const SENSOR &b)
{
    if (a.sqEnd == b.sqEnd) {
        return a.sqStart > b.sqStart;
    }
...
int main(void)
{
    ...
    for (SQWORD sqIdx = 0; sqIdx < sqN; sqIdx++) {
        ...
        vS.emplace_back(sqT, sqD);
    }

    sort(vS.begin(), vS.end());

    priority_queue<SENSOR, vector<SENSOR>, decltype(greaterEnd)> que {greaterEnd};
...
//        printf("cur %d %d\n", curS.sqStart, curS.sqEnd);

        if (curS.sqEnd < sqCurTime) {
            /* 間に合わない */
            ;
