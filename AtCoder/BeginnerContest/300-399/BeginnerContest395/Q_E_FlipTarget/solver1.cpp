...
void solveDijkstra(
    SQWORD sqFrom, 
    const vector<vector<EDGE_ST>> pvstEdges,
    vector<SQWORD> &cost)
{
...
int main(void)
{
    ...
    vector<vector<EDGE_ST>> vvsqEdge(sqN * 2);

    for (SQWORD sqIdx = 0; sqIdx < sqM; sqIdx++) {
        SQWORD sqU = inputSQWORD() - 1;
        SQWORD sqV = inputSQWORD() - 1;

        vvsqEdge[sqU].emplace_back(sqV, 1);
        vvsqEdge[sqV + sqN].emplace_back(sqU + sqN, 1);
    }
    for (SQWORD sqIdx = 0; sqIdx < sqN; sqIdx++) {
        vvsqEdge[sqIdx].emplace_back(sqN + sqIdx, sqX);
        vvsqEdge[sqN + sqIdx].emplace_back(sqIdx, sqX);
    }

    vector<SQWORD> vsqCost(sqN * 2, SQWORD_INF);
    solveDijkstra(0, vvsqEdge, vsqCost);

    printf("%lld\n", MIN(vsqCost[sqN - 1], vsqCost[sqN * 2 - 1]));
