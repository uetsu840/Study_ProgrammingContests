...
int main(void)
{
    ...
    vector<vector<SQWORD>> vvsqBoard(sqN, vector<SQWORD>(sqN));
    vector<SQWORD> vsqCountRow(sqN);
    vector<SQWORD> vsqCountCol(sqN);
    ...
    for (SQWORD sqRow = 0; sqRow < sqN; sqRow++) {
        for (SQWORD sqCol = 0; sqCol < sqN; sqCol++) {
            if (vvsqBoard[sqRow][sqCol] == 1) {
                sqAns += (vsqCountRow[sqRow] - 1) * (vsqCountCol[sqCol] - 1); 
            }
        }
    }
    printf("%lld\n", sqAns);
