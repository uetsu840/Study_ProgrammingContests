...
SQWORD dfs(SQWORD sqCur, SQWORD sqPrev, const vector<vector<SQWORD>> &vvsqEdge)
{
    SQWORD sqCount = 0;

    for (auto e: vvsqEdge[sqCur]) {
        if (e != sqPrev) {
            sqCount += dfs(e, sqCur, vvsqEdge);
        }
    }
    sqCount++;
...
int main(void)
{
    ...
    vector<SQWORD> vsqNodeCnt;
    for (auto e: vvsqEdge[0]) {
        SQWORD sqCount = dfs(e, 0, vvsqEdge);
        vsqNodeCnt.emplace_back(sqCount);
    }

    sort(vsqNodeCnt.begin(), vsqNodeCnt.end());

    SQWORD sqAns = 0;
    for (SQWORD sqIdx = 0; sqIdx < vsqNodeCnt.size() - 1; sqIdx++) {
        sqAns += vsqNodeCnt[sqIdx];
    }
    sqAns++;
    printf("%lld\n", sqAns);
