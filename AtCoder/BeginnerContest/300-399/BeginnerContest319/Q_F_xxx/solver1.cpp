...
int main(void)
{
    ...
    SQWORD sqAns = 0;
    for (SQWORD sqStartI = 0; sqStartI < sqN; sqStartI++) {
        vector<SQWORD> vsqVals;
        for (SQWORD sqIdxI = sqStartI; sqIdxI < sqN; sqIdxI++) {
            for (SQWORD sqIdxJ = 0; sqIdxJ < sqM; sqIdxJ++) {
        ...
        for (SQWORD sqIdxJ = 0; sqIdxJ < sqM; sqIdxJ++) {
            SQWORD sqOrder = mapVals[vvsqA[sqStartI][sqIdxJ]];
            sqAns += (sqOrder - sqIdxJ) + (sqIdxJ + 1) * (sqN - 1 - sqStartI);
