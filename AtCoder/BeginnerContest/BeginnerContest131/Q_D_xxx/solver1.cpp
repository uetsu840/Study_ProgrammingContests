...
int main()
{
    ...
    vector<WORK_ST> vecJobs;

    for (SQWORD sqIdx = 0; sqIdx < sqInput_N; sqIdx++) {
        SQWORD sqInput_a = inputSQWORD();
        SQWORD sqInput_b = inputSQWORD();

        vecJobs.emplace_back(sqInput_a, sqInput_b);
    }

    sort(vecJobs.begin(), vecJobs.end());
//    for (auto job: vecJobs) {
//        printf("%lld %lld\n", job.sqLimit, job.sqTime);
//    }

    SQWORD sqSumTime = 0;
    for (auto job: vecJobs) {
        sqSumTime += job.sqTime;
        if (job.sqLimit < sqSumTime) {
            printf("No\n");
            return 0;
        }
    }
