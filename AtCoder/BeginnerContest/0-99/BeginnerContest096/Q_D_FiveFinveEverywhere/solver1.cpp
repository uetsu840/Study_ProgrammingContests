...
int main(void)
{
    ...
    for (SQWORD sqIdx = 0; sqIdx < 4; sqIdx++) {
        sqSum += vlPrimes[sqIdx];
        vsqAns.emplace_back(vlPrimes[sqIdx]);
    }
    setSum4.insert(sqSum);

    for (SQWORD sqIdx = 4;; sqIdx++) {
        vector<SQWORD> sqAddSum;
        bool bIsAllSumPrime = false;
        for (auto s : setSum4) {
            if (setPrimes.find(s + vlPrimes[sqIdx]) != setPrimes.end()) {
                printf(">>>> %lld\n", s + vlPrimes[sqIdx]);
                bIsAllSumPrime = true;
            }
        }
        ...
        if (!bIsAllSumPrime) {
            printf("%lld\n", vlPrimes[sqIdx]);
            vsqAns.emplace_back(vlPrimes[sqIdx]);  
            for (SQWORD sqIdx1 = 0; sqIdx1 < sqIdx - 3; sqIdx1++) {
                for (SQWORD sqIdx2 = sqIdx1 + 1; sqIdx2 < sqIdx - 2; sqIdx2++) {
                    for (SQWORD sqIdx3 = sqIdx2 + 1; sqIdx3 < sqIdx - 1; sqIdx3++) {
                        for (SQWORD sqIdx4 = sqIdx3 + 1; sqIdx4 < sqIdx; sqIdx4++) {
