 ...
 *  セグメント木
 */
struct SEGMENT_NODE_SUM_ST {
    SQWORD sqVal;

    SEGMENT_NODE_SUM_ST(SQWORD a) : sqVal(a) {};
    SEGMENT_NODE_SUM_ST() : sqVal(0) {};
...
bool isGreaterX(const POINT &a, const POINT &b) {
    if (a.sqX == b.sqX) {
        return a.sqY < b.sqY;
    }
    return a.sqX < b.sqX;
...
bool isGreaterY(const POINT &a, const POINT &b) {
    if (a.sqY == b.sqY) {
        return a.sqX < b.sqX;
    }
    return a.sqY < b.sqY;
...
int main(void)
{
    ...
    /* 空間圧縮 */
    sort(vecstPoint.begin(), vecstPoint.end(), isGreaterY);
    for (SQWORD sqIdx = 0; sqIdx < sqN; sqIdx++) { 
        vecstPoint[sqIdx].sqIdxY = sqIdx;
    }
    sort(vecstPoint.begin(), vecstPoint.end(), isGreaterX);
    for (SQWORD sqIdx = 0; sqIdx < sqN; sqIdx++) { 
        vecstPoint[sqIdx].sqIdxX = sqIdx;
    }
    for (auto pnt: vecstPoint) {
        printf("pnt - pnt[%lld %lld] idx[%lld %lld]\n", pnt.sqX, pnt.sqY, pnt.sqIdxX, pnt.sqIdxY);
    }   

    /* 左側の点を動かしながら最大を探す */
    SQWORD sqAns = MAX_SQWORD;
    for (SQWORD sqIdxL = 0; sqIdxL < sqN - 1; sqIdxL++) {
        vector<SEGMENT_NODE_SUM_ST> vecstSegNode(sqN, 0);
        SegmentTree<SEGMENT_NODE_SUM_ST> seg(vecstSegNode);
        for (SQWORD sqIdxR = 0; sqIdxR < sqN; sqIdxR++) {
            seg.update(vecstPoint[sqIdxR].sqIdxY, 1);

            SQWORD sqYIdx1 = vecstPoint[sqIdxL].sqIdxY;
            SQWORD sqYIdx2 = vecstPoint[sqIdxR].sqIdxY;
            SQWORD sqYIdxH = max(sqYIdx1, sqYIdx2);
            SQWORD sqYIdxL = min(sqYIdx1, sqYIdx2);
            
            SQWORD sqPntCnt = seg.GetValue(sqYIdxL, sqYIdxH);

            printf("[%lld %lld] %lld\n", sqIdxL, sqIdxR, sqPntCnt);

            if (sqK - 2 < sqPntCnt) {
                SQWORD sqX = abs(vecstPoint[sqIdxL].sqX - vecstPoint[sqIdxR].sqX);
                SQWORD sqY = abs(vecstPoint[sqIdxL].sqY - vecstPoint[sqIdxR].sqY);
                printf("->%lld %lld\n", sqX, sqY);
                sqAns = min(sqAns, sqX * sqY);
            }
        }
    }
    printf("%lld\n", sqAns);
