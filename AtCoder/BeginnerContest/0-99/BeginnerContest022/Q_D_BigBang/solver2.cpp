...
static DOUBLE getMaxDist(vector<VECTOR_2D> &vConvex)
{
    ...
    for (SQWORD sqIdxT = 0; sqIdxT < sqNumPnt; sqIdxT++) {
        DOUBLE dNormCur = (vConvex[sqIdxT] - vConvex[sqIdxF]).norm();
        while (1) {
            SQWORD sqPntNext = (sqIdxF + 1) % sqNumPnt;
            DOUBLE dNormNext = (vConvex[sqIdxT] - vConvex[sqPntNext]).norm();
            if (dNormCur < dNormNext) {
                sqIdxF = sqPntNext;
                dNormCur = dNormNext;
            } else {
                break;
        ...
        dMaxNorm = max(dNormCur, dMaxNorm);
    }
