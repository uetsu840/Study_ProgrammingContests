...
{
    /**
     * dp(i)[j] : idx i (i-1番目) までで a[i] <= a[i+1] を満たし、最大がjとなる分割数
    ...
    for (SDWORD lIdx = 0; lIdx <= sqDiv; lIdx++) {
        for (SQWORD sqSumIdx = 1; sqSumIdx <= sqSum; sqSumIdx++) {
...
int main(void)
{
    ...
    SQWORD sqAns = getDivNum(lInput_n, lInput_k);
    printf("%lld\n", sqAns);
