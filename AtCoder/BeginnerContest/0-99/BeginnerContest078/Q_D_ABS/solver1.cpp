...
int main(void)
{
    ...
    SQWORD sqAns;
    if (1 == sqN) {
        sqAns = abs(sqW - vecsqA[0]);
    } else {
        /**
         *  Xさんが1枚残して引く    Xさん last-1, Yさん last  
         *  Xさんが全部引く         Xさん last, Yさん W
         */
        SQWORD sqLast    = vecsqA[sqN - 1]; 
        SQWORD sqPreLast = vecsqA[sqN - 2];

        sqAns = max(abs(sqPreLast - sqLast), abs(sqLast - sqW));
    }
