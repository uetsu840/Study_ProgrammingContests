    ...
    SQWORD sqCur)
{
    ...
    for (auto e: vvEdge[sqCur]) {
        if (!vbIsVisited[e]) {
            sqEdgeCnt++;
            SQWORD sqPathCnt = dfs(vbIsVisited, vvEdge, e);
            if (-1 == sqPathCnt) {
                return -1;
            }
            if (1000000 <= sqEdgeCnt) {
                return -1;
            }
            sqRet += sqPathCnt;
        }
...
int main(void)
{
    ...
    SQWORD sqAns = dfs(vbIsVisited, vvsqEdge, 0);
    if ((-1 == sqAns) || (1000000 <= sqAns)) {
        printf("1000000\n");
    } else {
        printf("%lld\n", sqAns);
    }
