...
int main(void)
{
    ...
    if (!(('A' <= strS[0]) && (strS[0] <= 'Z'))) {
        printf("No\n");
        return 0;
    }

    SQWORD sqSum = 0;
    for (SQWORD sqPos = 1; sqPos <= 6; sqPos++) {
        sqSum *= 10;
        if (!(('0' <= strS[sqPos]) && (strS[sqPos] <= '9'))) {
            printf("No\n");
            return 0;
        }
        SQWORD sqNum = strS[sqPos] - '0';
        sqSum += sqNum;
    }
    if (!((100000 <= sqSum) && (sqSum <= 999999))) {
            printf("No\n");
            return 0;
    }
    
    if (!(('A' <= strS[7]) && (strS[7] <= 'Z'))) {
        printf("No\n");
        return 0;
    }
