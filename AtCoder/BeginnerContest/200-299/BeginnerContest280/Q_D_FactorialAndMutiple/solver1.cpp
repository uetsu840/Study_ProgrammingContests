...
int main(void)
{
    ...
    vector<pair<SQWORD, SQWORD>> vpPrimes;

    calcPrimeFactorication(sqK, vpPrimes);

    SQWORD sqAns = 0;
    for (auto num: vpPrimes) {
        SQWORD sqNo = num.first;
        SQWORD sqFact = num.second;
        SQWORD sqMul = 1;
        SQWORD sqTtlMul = 0;
        for (;;) {
            SQWORD sqCurNo = sqMul * sqNo;             
            for (;;) {
                if (0 == sqCurNo % sqNo) {
                    sqCurNo /= sqNo;
                    sqTtlMul++;
