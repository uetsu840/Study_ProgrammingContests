...
static SQWORD backtrack(
    stack<SQWORD> &lifoPath, 
    const vector<SQWORD> &vsqC,
    SQWORD sqTail)
{
    SQWORD sqMinCost = MAX_COST;
    for (;;) {
        if (lifoPath.empty()) {
            return 0;
        }
        SQWORD sqBackTrack = lifoPath.top();
        sqMinCost = MIN(sqMinCost, vsqC[sqBackTrack]);
        lifoPath.pop();
        if (sqBackTrack == sqTail) {
            return sqMinCost;
        }
    }
}
...
int main(void)
{
    ...
    for (SQWORD sqStart = 0; sqStart < sqN; sqStart++) {
        if (!vbVisited[sqStart]) {
            stack<SQWORD> lifoPath;
            SQWORD sqCur = sqStart;
            for (;;) {
                lifoPath.push(sqCur);
                vbVisited[sqCur] = true;
                SQWORD sqNext = vsqX[sqCur];
                if (vbVisited[sqNext]) {
                    /* backtrack */
                    sqAns += backtrack(lifoPath, vsqC, sqNext);
                    break;
                } else {
                    /* search */
                    sqCur = sqNext;
