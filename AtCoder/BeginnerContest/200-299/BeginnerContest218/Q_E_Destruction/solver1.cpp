...
struct EDGE_ST {
    ...
    EDGE_ST(SQWORD a, SQWORD b, SQWORD c): sqA(a), sqB(b), sqC(c) {};
};

bool operator< (const EDGE_ST &a, const EDGE_ST &b) {
    return a.sqC < b.sqC;
}
...
int main(void)
{
    ...
    vector<EDGE_ST> vstEdge;

    SQWORD sqEdgeCostSum = 0;
    for (SQWORD sqIdx = 0; sqIdx < sqM; sqIdx++) {
        ...
        sqA--;
        sqB--;
        vstEdge.emplace_back(sqA, sqB, sqC);
        sqEdgeCostSum += sqC;
    }
    sort(vstEdge.begin(), vstEdge.end());

    ST_UNION_FIND uf;
    uf.initUnionFind(sqN);

    /* 負の辺はすべて追加しておく(取る必要がない) */
    SQWORD sqCostSum = 0;
    for (SQWORD sqIdx = 0; sqIdx < vstEdge.size(); sqIdx++) {
        auto e = vstEdge[sqIdx];
//        printf("%lld\n", e.sqC);
        if (e.sqC < 0) {
            uf.ufUnite(e.sqA, e.sqB);
            sqCostSum += e.sqC;
        }
    }
