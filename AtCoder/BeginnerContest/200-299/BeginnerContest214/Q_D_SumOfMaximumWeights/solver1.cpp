...
struct ST_EDGE {
    ...
    ST_EDGE(SQWORD u, SQWORD v, SQWORD w): sqU(u), sqV(v), sqW(w) {};
};

bool operator< (const ST_EDGE &a, const ST_EDGE &b) {
    return a.sqW < b.sqW;
}
...
int main(void)
{
    ...
    for (SQWORD sqIdx = 0; sqIdx < sqN - 1; sqIdx++) {
        ...
        vstEdge.emplace_back(sqU, sqV, sqW);
    }
    ...
    SQWORD sqAns = 0;
    SQWORD sqProcCount = 0;
    for (auto e: vstEdge) {
        SQWORD sqCount1 = uf.ufGetCnt(e.sqU);
        SQWORD sqCount2 = uf.ufGetCnt(e.sqV);

        sqProcCount++;

        sqAns += sqCount1 * sqCount2 * e.sqW;
        uf.ufUnite(e.sqU, e.sqV);
    }
