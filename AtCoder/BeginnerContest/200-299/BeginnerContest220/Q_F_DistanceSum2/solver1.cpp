...
SQWORD dfs(SQWORD sqFrom, SQWORD sqCur)
{
    printf("--- %lld\n", sqCur);
    SQWORD sqNodeCnt = 1;
...
int main(void)
{
    ...
    for (auto n: vmapNodeCnts) {
        for (auto c: n) {
            printf("%lld(%lld) ", c.first, c.second);
        }
        printf("\n");
    }

    for (auto e: vstEdges) {
        printf("%lld %lld\n", vmapNodeCnts[e.sqFrom][e.sqTo], vmapNodeCnts[e.sqTo][e.sqFrom]);
    }
