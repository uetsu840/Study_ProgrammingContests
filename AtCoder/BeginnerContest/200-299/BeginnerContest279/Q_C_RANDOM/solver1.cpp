...
int main(void)
{
    ...
    vector<string> vStrSR(sqW);
    vector<string> vStrTR(sqW);
    for (SQWORD sqIdx = 0; sqIdx < sqH; sqIdx++) {
        string strS;
        cin >> strS;
        for (SQWORD sqPos = 0; sqPos < sqW; sqPos++) {
            vStrSR[sqPos].push_back(strS[sqPos]);
        }
    }
    for (SQWORD sqIdx = 0; sqIdx < sqH; sqIdx++) {
        string strT;
        cin >> strT;
        for (SQWORD sqPos = 0; sqPos < sqW; sqPos++) {
            vStrTR[sqPos].push_back(strT[sqPos]);
        }
    }

    multiset<string> setStrS;
    for (auto s: vStrSR) {
        setStrS.insert(s);
    }

    for (auto t: vStrTR) {
        auto it = setStrS.find(t);
        if (it != setStrS.end()) {
            setStrS.erase(it);
        } else {
            printf("No\n");
