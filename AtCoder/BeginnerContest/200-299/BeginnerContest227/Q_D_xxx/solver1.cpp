...
//        printf("---%lld\n", sqA);
    ...
    vector<SEGMENT_NODE_ST> vSegNodes;
    for (SQWORD sqIdx = 0; sqIdx < sqN; sqIdx++) {
        vSegNodes.emplace_back(SEGMENT_NODE_ST(vsqA[sqIdx]));
    }
    LazySegmentTree tree(vSegNodes);

    SQWORD sqCur = 0;
    SQWORD sqAns = 0;
    while(1) {
        SQWORD sqMinVal = tree.FindVal(sqCur, sqCur + 1).stVal.sqVal;
//        printf("--->> %lld\n", sqMinVal);
        sqAns += sqMinVal;
        tree.UpdateValue(sqCur, sqCur + sqK, -sqMinVal);
        sqCur++;
        while (0 == tree.FindVal(sqCur, sqCur + 1).stVal.sqVal) {
//            printf("--->>>>> %lld\n", sqCur);
            sqCur++;
        }
