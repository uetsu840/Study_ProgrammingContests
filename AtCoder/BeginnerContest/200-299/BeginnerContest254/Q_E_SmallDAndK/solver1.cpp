...
int main(void)
{
    ...
    static vector<SQWORD> avsqEdge[MAX_N];
    ...
    for (SQWORD sqIdx = 0; sqIdx < sqM; sqIdx++) {
        ...
        avsqEdge[sqA].emplace_back(sqB);
        avsqEdge[sqB].emplace_back(sqA);
    }
    ...
    for (SQWORD sqQueryIdx = 0; sqQueryIdx < sqQ; sqQueryIdx++) {
        ...
        vector<SQWORD> vsqDist(sqN, COST_MAX);
        ...
        queSearch.push(sqX);
        for (;;) {
            SQWORD sqCurNode = queSearch.front();
            queSearch.pop();

            SQWORD sqCurDist = vsqDist[sqCurNode];
            ...
            sqAns += (sqCurNode + 1);
            for (auto e: avsqEdge[sqCurNode]) {
                if (vsqDist[e] == COST_MAX) {
                    vsqDist[e] = sqCurDist + 1;
                    queSearch.push(e);
                }
            }
            if (queSearch.empty()) {
