...
struct ENTRY_ST {
    ...
    ENTRY_ST(SQWORD n, SQWORD p): sqNum(n), sqPos(p) {};
};

bool greaterNum(const ENTRY_ST &a, const ENTRY_ST &b)
{
    if (a.sqNum == b.sqNum) {
...
int main(void)
{
    ...
    vector<ENTRY_ST> vstEntry;

    for (SQWORD sqIdx = 0; sqIdx < sqN; sqIdx++) {
        SQWORD sqA = inputSQWORD();
        vstEntry.emplace_back(sqA, sqIdx);
    }

    sort(vstEntry.begin(), vstEntry.end(), greaterNum);

    BinaryIndexedTree<MODINT> bit(sqN);

    MODINT miAns = 0;
    MODINT miTtl = 0;
    for (auto e: vstEntry) {
        MODINT miPosVal = MODINT(2).pow(e.sqPos);
        bit.Add(e.sqPos + 1, miPosVal);
        MODINT miSum = bit.Sum(e.sqPos + 1);
        miTtl += miPosVal;

        MODINT miSegSum = (miTtl - miSum) / (miPosVal);
