...
struct BALL_ENTRY {
    ...
    BALL_ENTRY(SQWORD b, SQWORD v): sqBaseOffset(b), sqVal(v) {};
};

bool operator< (const BALL_ENTRY &a, const BALL_ENTRY &b)
{
        ...
        case 1:
            {
                ...
                setBalls.insert(sqVal - sqOffsetSum);
            }
            break;
        case 2:
            {
                SQWORD sqOfs = inputSQWORD();
                sqOffsetSum += sqOfs;
            }
            break;
        case 3:
            {
                auto itPicked = setBalls.begin();
                vsqAns.emplace_back(*itPicked + sqOffsetSum);
                setBalls.erase(itPicked);
            }
            break;
        default:
            break;
