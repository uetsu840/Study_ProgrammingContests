...
static SQWORD s_aasqCombination[MAX_LENGTH * 2 + 1][MAX_LENGTH * 2 + 1];
...
static void prepareComb()
{
    ...
    for (SQWORD sqM = 1; sqM <= MAX_LENGTH * 2; sqM++) {
        SQWORD sqComb = sqM;
...
int main(void)
{
    ...
    SQWORD sqRestA = sqA;
    SQWORD sqRestB = sqB;
    SQWORD sqSum = sqA + sqB;
    SQWORD sqRestK = sqK;
    string strAns;

    for (SQWORD sqIdx = 0; sqIdx < sqSum; sqIdx++) {
        SQWORD sqCntA;
        if (0 == sqRestA) {
            sqCntA = 0;
        } else {
            sqCntA = s_aasqCombination[sqSum - sqIdx - 1][sqRestA - 1];
        }
        if (sqRestK <= sqCntA) {
            /* sqIdx文字目はa */
            strAns += 'a';
