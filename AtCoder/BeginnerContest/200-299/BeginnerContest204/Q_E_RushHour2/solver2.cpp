...
#endif
    while (!pqArrivalInfo.empty()) {
        ST_CITY_ARRIVAL_INFO p = pqArrivalInfo.top();
        pqArrivalInfo.pop();

        SDWORD v = p.sqCityIdx;
        if (p.sqTime <= vsqMinArriveTime[v]) {
            for (SDWORD lIdx = 0; lIdx < vstTimeTbl[v].size(); lIdx++) {
                auto d = vstTimeTbl[v][lIdx];

                SQWORD sqArriveTime = getArrivalTime(p.sqTime, d.sqC, d.sqD);
                if (vsqMinArriveTime[d.sqTo] > sqArriveTime) {
                    vsqMinArriveTime[d.sqTo] = sqArriveTime;
                    pqArrivalInfo.push({d.sqTo, sqArriveTime});
                }
            }
        }
    }
