...
int main(void)
{
    ...
    SQWORD sqAns = 0;
    sort(vstPoints.begin(), vstPoints.end(), greaterX);
    for (SQWORD sqIdx = 0; sqIdx < sqN - 1; sqIdx++) {
        SQWORD sqDist = MIN(ABS(vstPoints[sqIdx].sqX - vstPoints[sqIdx + 1].sqX),
                            ABS(vstPoints[sqIdx].sqY - vstPoints[sqIdx + 1].sqY));
        printf("%lld\n", sqDist);
        sqAns = MAX(sqAns, sqDist);
    }

    sort(vstPoints.begin(), vstPoints.end(), greaterY);
    for (SQWORD sqIdx = 0; sqIdx < sqN - 1; sqIdx++) {
        SQWORD sqDist = MIN(ABS(vstPoints[sqIdx].sqX - vstPoints[sqIdx + 1].sqX),
                            ABS(vstPoints[sqIdx].sqY - vstPoints[sqIdx + 1].sqY));
        sqAns = MAX(sqAns, sqDist);
