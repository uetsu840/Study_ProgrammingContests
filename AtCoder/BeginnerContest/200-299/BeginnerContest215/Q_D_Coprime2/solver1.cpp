...
int main(void)
{
    ...
    unordered_set<SQWORD> setPrimes;
    for (SQWORD sqIdx = 0; sqIdx < sqN; sqIdx++) {
        SQWORD sqA = inputSQWORD();
        vector<pair<SQWORD, SQWORD>> vsqPrimes;
        calcPrimeFactorication(sqA, vsqPrimes);

        for (auto p: vsqPrimes) {
            setPrimes.insert(p.first);
        }
    }

    vector<bool> vsqIsDividable(sqM, false);
    for (auto p: setPrimes) {
        for (SQWORD sqIdx = 1; p * sqIdx <= sqM; sqIdx++) {
            vsqIsDividable[p * sqIdx] = true;
        }
    }

    vector<SQWORD> vsqAns;
    for (SQWORD sqNum = 1; sqNum <= sqM; sqNum++) {
        if (!vsqIsDividable[sqNum]) {
