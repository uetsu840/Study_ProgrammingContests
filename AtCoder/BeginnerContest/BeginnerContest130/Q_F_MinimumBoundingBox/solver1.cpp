    ...
    DOUBLE &dSect1)
{
    SQWORD sqX_P_Btm;
    if (vsqStp.empty()) {
        SQWORD sqInc = *(vsqInc.end() - 1);
        SQWORD sqDec = *(vsqDec.end() - 1);
        dSect0 = ((DOUBLE)sqDec - (DOUBLE)sqInc) / 2.0;
        dSect1 = dSect0;
    } else {
        if (vsqInc.empty()) {
            SQWORD sqStp = *(vsqStp.end() - 1);
            SQWORD sqDec = *(vsqDec.end() - 1);
            dSect0 = (DOUBLE)(sqDec - sqStp);
            dSect1 = 0.0;
        } else if (vsqDec.empty()) {
            SQWORD sqStp = *(vsqStp.end() - 1);
            SQWORD sqInc = *(vsqInc.end() - 1);
            dSect0 = 0.0;
            dSect1 = (DOUBLE)(sqStp - sqInc);
        } else {
