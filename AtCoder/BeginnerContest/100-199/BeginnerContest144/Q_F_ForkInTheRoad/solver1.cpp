...
int main(void)
{
    ...
    DOUBLE s_adDistExValF[N_MAX_NODE+1];
    ...
    for (SQWORD sqNode = 1; sqNode <= sqN; sqNode++) {
        ...
        for (auto next: s_avecsqOutEdge[sqNode]) {
            ...
            s_adDistExValF[next] += dExValFCur + dEdgeExVal;
        }
    }

    /* 各頂点から出口までの期待値を求める */
    DOUBLE s_adDistExValR[N_MAX_NODE+1];
    for (SQWORD sqStart = 1; sqStart <= sqN; sqStart++) {
        ...
        for (SQWORD sqNode = sqStart; sqNode <= sqN; sqNode++) {
            ...
            for (auto next: s_avecsqOutEdge[sqNode]) {
                DOUBLE dEdgeExVal = s_adProba[sqNode] / (DOUBLE)sqOutEdgeNum;
                vecdDistExVal[next] += dExValCur + dEdgeExVal;
            }
        }
    ...
    for (SQWORD sqNode = 1; sqNode <= sqN; sqNode++) {
        for (auto next: s_avecsqOutEdge[sqNode]) {
            ...
            DOUBLE dCurScore = (1.0 / (1.0 - dProba)) * (dEx)
            if (< dScoreMin) {
                printf("%lld %lld %lf \n", sqNode, next, dExValTtl);
                dExValMax = dExValTtl;
                dProbaExValMax = ;
                pairsqLargestEdge = make_pair(sqNode, next);
            }                              
        }
    }

    printf("%0.10f\n", s_adDistExValAns[sqN]);
