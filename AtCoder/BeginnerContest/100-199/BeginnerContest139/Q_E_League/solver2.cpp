...
int main(void)
{
    ...
    /* initialize */
    static SDWORD s_alEntry[MAX_PARTICIPANT + 1];
    for (SDWORD lNo = 1; lNo <= lN; lNo++) {
    ...
    for (;;) {
        ...
        for (SDWORD lNo = 1; lNo <= lN; lNo++) {
            if (-1 != s_alEntry[lNo]) {
//                printf("----%d %d\n", lNo, s_alEntry[lNo]);
                if (-1 != s_alEntry[s_alEntry[lNo]]) {
                    if (s_alEntry[s_alEntry[lNo]] == lNo) {
                        veclRemoveCnt.emplace_back(s_alEntry[lNo]);
                        veclRemoveCnt.emplace_back(lNo);
//                        printf("%d %d\n", s_alEntry[lNo], lNo);
                        s_alEntry[s_alEntry[lNo]] = -1;
                        s_alEntry[lNo] = -1;
                    }
                }
            }
