...
struct ST_TIMETABLE {
    ...
    ST_TIMETABLE(SQWORD d, SQWORD k, SQWORD t): sqDst(d), sqK(k), sqT(t) {};
};
...
inline SQWORD divUp(SQWORD val, SQWORD d) 
{
    return ((val + (d - 1)) / d) * d;
}
...
int main(void)
{
    ...
    for (SQWORD sqIdx = 0; sqIdx < sqM; sqIdx++) {
        ...
        vstTimeTbl[sqA].emplace_back(sqB, sqK, sqT);
        vstTimeTbl[sqB].emplace_back(sqA, sqK, sqT);
    }
    ...
    pqArrivalInfo.push(ST_CITY_ARRIVAL_INFO{sqX, 0});
    while (!pqArrivalInfo.empty()){
        ...
        if (vsqMinArriveTime[stCur.sqCityIdx] == MAX_SQWORD) {
            ...
            for (auto d: vstTimeTbl[stCur.sqCityIdx]) {
                ...
                SQWORD sqArriveTime = divUp(stCur.sqTime, d.sqK) + d.sqT;
    ...
    if (MAX_SQWORD == vsqMinArriveTime[sqY]) {
        printf("-1\n");
    } else {
        printf("%lld\n", vsqMinArriveTime[sqY]);
    }
    return 0;
}
