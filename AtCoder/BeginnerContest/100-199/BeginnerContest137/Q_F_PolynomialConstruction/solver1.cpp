...
int main(void)
{
    ...
    for (SQWORD sqIdx = 0; sqIdx < sqP; sqIdx++) {
        vecsqA.emplace_back(inputSQWORD());
    }

    MODINT::Init(sqP);
    vector<MODINT> vecmsqB(sqP);
    Combination comb(sqP-1);

    for (SQWORD sqIdxA = 0; sqIdxA < sqP; sqIdxA++) {
        if (0 < vecsqA[sqIdxA]) {
            vecmsqB[0] = vecmsqB[0] + 1;
            SQWORD sqA(vecsqA[sqIdxA]);
            MODINT msqPowI(1);
            for (SQWORD sqJ = sqP-1; 0 <= sqJ; sqJ--) {
                MODINT msqB_l = msqPowI * comb.GetVal(sqJ);
                vecmsqB[sqJ] = vecmsqB[sqJ] - msqB_l;
                msqPowI *= (sqP - sqIdxA);
            }
        }
    }
