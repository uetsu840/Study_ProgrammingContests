...
struct SEGMENT_VAL_ST {
    ...
    {
        SEGMENT_VAL_ST ret;
        ret.sqVal = (a.sqVal + b.sqVal) % ANS_MOD;
        return ret;
    }
};
...
struct RANGE_ST {
    SQWORD sqL;
    SQWORD sqR;
    RANGE_ST (SQWORD l, SQWORD r): sqL(l), sqR(r) {};
};
...
int main(void)
{
    ...
    vector<RANGE_ST> vecRange;
    for (SQWORD sqIdx = 0; sqIdx < sqK; sqIdx++) {
        SQWORD sqL = inputSQWORD();
        SQWORD sqR = inputSQWORD();

        vecRange.emplace_back(sqL, sqR);
    }

    vector<SEGMENT_NODE_ST> vstSegNode(sqN + 1);
    vstSegNode[1] = 1;
    LazySegmentTree seg(vstSegNode);
    for (SQWORD sqFromIdx = 1; sqFromIdx < sqN; sqFromIdx++) {
        SQWORD sqVal = seg.FindVal(sqFromIdx, sqFromIdx + 1).stVal.sqVal;

//        printf("%lld %lld\n", sqFromIdx, sqVal);

        for (auto range: vecRange) {
            seg.UpdateValue(sqFromIdx + range.sqL, sqFromIdx + range.sqR + 1, SEGMENT_VAL_ST(sqVal));
        }
    }

    printf("%lld\n", seg.FindVal(sqN, sqN + 1).stVal.sqVal);
