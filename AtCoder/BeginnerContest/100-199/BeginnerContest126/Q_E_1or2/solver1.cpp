...
int main(void)
{
    ...
    static ST_UNION_FIND stUf;
    initUnionFind(&stUf, sqInput_N);

    SQWORD sqConstraintCount = 0;
    for (SQWORD sqIdx = 0; sqIdx < sqInput_M; sqIdx++) {
        ...
        if (ufIsSame(&stUf, sqInput_X, sqInput_Y)) {
            ;
        } else {
            sqConstraintCount++;
            ufUnite(&stUf, sqInput_X, sqInput_Y);
        }
    }

    printf("%lld\n", sqInput_N - sqConstraintCount);
