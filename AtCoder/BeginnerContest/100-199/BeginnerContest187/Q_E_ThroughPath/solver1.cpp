...
struct EDGE_ST {
    ...
    EDGE_ST(SQWORD t, SQWORD i, DIRECTION d): sqTo(t), sqEdgeId(i), eD(d) {};
};
...
vector<vector<EDGE_ST>> vvEdges;
vector<EDGE_INFO_ST> vEdgeInfo;
vector<SQWORD> vsqVerticeOrder;

static void dfs(SQWORD sqFrom, SQWORD sqCur) {
    vsqVerticeOrder.emplace_back(sqCur);

    for (auto v: vvEdges[sqCur]) {
        if (v.sqTo != sqFrom) {
            SQWORD sqOrderLow = vsqVerticeOrder.size() - 1;

            /* dfs */
            dfs(sqCur, v.sqTo);

            SQWORD sqOrderHigh = vsqVerticeOrder.size() - 1;

            /* cofigure info */
            if (v.eD == DIR_START) {
                vEdgeInfo[v.sqEdgeId].sqBegin = sqOrderLow;
                vEdgeInfo[v.sqEdgeId].sqEnd = sqOrderHigh;
            } else {
                vEdgeInfo[v.sqEdgeId].sqBegin = sqOrderHigh;
                vEdgeInfo[v.sqEdgeId].sqEnd = sqOrderLow;
            }
        }
    }
}
...
int main(void)
{
    ...
    for (SQWORD sqIdx = 0; sqIdx < sqN-1; sqIdx++) {
        ...
        vvEdges[sqA].emplace_back(sqB, sqIdx, DIR_START);
        vvEdges[sqB].emplace_back(sqA, sqIdx, DIR_END);
    }

    dfs(-1, 0);

    vector<SEGMENT_NODE_ST> vstSegNodes(sqN);
    ...
    for (SQWORD sqQueryIdx = 0; sqQueryIdx < sqQ; sqQueryIdx++) {
        ...
        SQWORD sqBegin = vEdgeInfo[sqE].sqBegin;
        SQWORD sqEnd   = vEdgeInfo[sqE].sqEnd; 
        ...
        if (sqBegin < sqEnd) {
            seg.UpdateValue(sqBegin + 1, sqEnd + 1, SEGMENT_VAL_ST(sqQ));
        } else {
            seg.UpdateValue(0, sqEnd + 1, SEGMENT_VAL_ST(sqQ));
            seg.UpdateValue(sqBegin + 1, sqN, SEGMENT_VAL_ST(sqQ));
        }
    }

    vector<SQWORD> vsqAns(sqN, 0);
    for (SQWORD sqIdx = 0; sqIdx < sqN; sqIdx++) {
        vsqAns[vsqVerticeOrder[sqIdx]] = seg.FindVal(sqIdx, sqIdx+1).stVal.sqVal;
    }
