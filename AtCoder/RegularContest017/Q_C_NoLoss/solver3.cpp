...
DWORD lookup(vector<SQWORD>& v, const SQWORD& x)
{
    pair<vector<SQWORD>::iterator, vector<SQWORD>::iterator> range;
...
DWORD solve(DWORD dwInput_X, DWORD dwInput_N) {
    ...
    for (DWORD dwIdx = 0; dwIdx < ArrayLength(s_aqwSumsU); dwIdx++) {
        QWORD qwSum = 0;
        DWORD dwPos = 0;
    ...
    for (DWORD dwIdx = 0; dwIdx < ArrayLength(s_aqwSumsD); dwIdx++) {
        SQWORD qwSum = 0;
        DWORD dwPos = 0;
        for (DWORD dwBit = dwIdx; dwBit != 0; dwBit >>= 1, dwPos++) {
            if (dwBit & 1) {
                qwSum += (SQWORD)s_adwSize[dwPos + 16];
            }
        }
    ...
    if (dwInput_N <= 16) {
        static  vector<SQWORD> v(s_aqwSumsU, s_aqwSumsU + (1 << dwInput_N));
        sort(v.begin(), v.end());
        dwSum = lookup(v, dwInput_X);
    } else {
        static  vector<SQWORD> v(s_aqwSumsU, s_aqwSumsU + 65536);
        sort(v.begin(), v.end());
        for (DWORD dwIdx = 0; dwIdx < (1<<(dwInput_N - 16)); dwIdx++) {
            if (s_aqwSumsD[dwIdx] <= (SQWORD) dwInput_X) {
                dwSum += lookup(v, (SQWORD)dwInput_X - s_aqwSumsD[dwIdx]); 
            }
        }
    }
    return dwSum;
