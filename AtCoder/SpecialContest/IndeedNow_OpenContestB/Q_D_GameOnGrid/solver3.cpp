...
int main()
{
    ...
    for (SDWORD lRow = 0; lRow < lInput_H; lRow++) {
        for (SDWORD lCol = 0; lCol < lInput_W; lCol++) {
            ...
            sqTtlPnt += s_aalPoint[lRow][lCol];
        }
    }

    for (SDWORD lRow = 0; lRow < lInput_H; lRow++) {
        for (SDWORD lCol = 0; lCol < lInput_W; lCol++) {
            if ((lRow != lInput_Sy - 1) || (lCol != lInput_Sx - 1)) {
                SDWORD lMaxBonus = 0;
                for (auto dir : vplDir) {
                    SDWORD lFromRow = lRow + dir.first;
                    SDWORD lFromCol = lCol + dir.second;

                    if ((0 <= lFromRow)
                        && (0 <= lFromCol) 
                        && (lFromRow <= lInput_H - 1)
                        && (lFromCol <= lInput_W - 1)) {
                        lMaxBonus = max(lMaxBonus, s_aalPoint[lRow][lCol] * s_aalPoint[lFromRow][lFromCol]);
                    }
                }
                printf(">> %d\n", lMaxBonus);
                sqTtlPnt += lMaxBonus;
