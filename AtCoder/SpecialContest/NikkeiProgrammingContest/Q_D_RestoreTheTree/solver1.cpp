...
int main(void)
{
    ...
    static vector<pair<DWORD, DWORD>> avEdges[MAX_POINTS];
    static vector<pair<DWORD, DWORD>> avRevEdges[MAX_POINTS];
    ...
    for (DWORD dwIdx = 1; dwIdx <= dwEdgeNum; dwIdx++) {
        ...
        avEdges[dwInput_A].push_back(make_pair(dwInput_B, 0));
        avRevEdges[dwInput_B].push_back(make_pair(dwInput_A, 0));
    }
    ...
    while(1) {
        ...
        dwCurPos = avRevEdges[dwCurPos][0].first;
    }
        ...
        for (DWORD dwPnt = 1; dwPnt <= dwInput_N; dwPnt++) {
            if (adwDist[dwPnt] != DIST_MIN) {
                for (DWORD dwEdgeIdx = 0; dwEdgeIdx < avEdges[dwPnt].size(); dwEdgeIdx++) {
                    DWORD dwNextPnt = avEdges[dwPnt][dwEdgeIdx].first;
    ...
    for (DWORD dwPnt = 1; dwPnt <= dwInput_N; dwPnt++) {
        ...
        if (0 == dwEdgeNum) {
            ...
            for (DWORD dwEdgeIdx = 0; dwEdgeIdx < dwEdgeNum; dwEdgeIdx++) {
                DWORD dwPrevPntDist = adwDist[avRevEdges[dwPnt][dwEdgeIdx].first];
                if (dwPrevPntDist + 1 == adwDist[dwPnt]) {
                    printf("%d\n", avRevEdges[dwPnt][dwEdgeIdx].first);
                    break;
                }
            }
        }
