...
int main()
{
    ...
    DWORD dwRefIdx = 0;
    bool bMatch = false;
    bool bMatchPrev = true;
    bool bNextMatchInh = false;
    for (DWORD dwIdx = 0; dwIdx < strlen(s_acString); dwIdx++) {
        if (s_acString[dwIdx] == s_acString[dwIdx+1]) {
            bMatch = false;
        } else if (s_acString[dwIdx] == s_acRefString[dwRefIdx]) {
            bMatch = true;
            dwRefIdx++;
        } else {
            if (bNextMatchInh) {
        ...
        if (bMatch && !bMatchPrev) {
            bNextMatchInh = true;
        }
        bMatchPrev = bMatch;
    }
    if (dwRefIdx == strlen(s_acRefString)) {
        printf("YES\n");
    } else {
        printf("NO\n");
    }
