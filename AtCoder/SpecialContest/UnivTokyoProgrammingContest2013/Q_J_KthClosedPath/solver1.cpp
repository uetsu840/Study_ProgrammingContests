...
bool operator< (const PATH_INFO &a, const PATH_INFO &b) {
...
int main(void)
{
    ...
    for (auto e: vvEdge[0]) {
        pq.push(PATH_INFO{e.sqTo, e.sqDist});
    }

    SQWORD sqCnt = 0;
    for (;;) {
        auto cur_path = pq.top();
        pq.pop();
//        printf("\n>> [%lld, %lld]\n", cur_path.sqPos, cur_path.sqLen);
        ...
        for (auto e: vvEdge[cur_path.sqPos]) {
//            printf("%lld ", e.sqTo);
            pq.push(PATH_INFO{cur_path.sqLen + e.sqDist, e.sqTo});
        }
    }
