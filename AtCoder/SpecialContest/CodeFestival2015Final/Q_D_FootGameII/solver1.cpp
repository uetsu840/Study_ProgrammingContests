...
struct BUTTON {
    ...
    BUTTON (SQWORD s, SQWORD t) : sqS(s), sqT(t) {}; 
};
...
int main(void)
{
    ...
    for (SQWORD sqIdx = 0; sqIdx < sqN; sqIdx++) {
        ...
        vecButton.emplace_back(sqS, sqT);
    }

    vector<SEGMENT_NODE_ST> vecstInit(MAX_T);
    for (SQWORD sqIdx = 0; sqIdx < vecstInit.size(); sqIdx++) {
        vecstInit[sqIdx].stVal.setVal(0);
    }
    LazySegmentTree seg(vecstInit); 

    for (auto b: vecButton) {
        SEGMENT_VAL_ST segAdd(1);
        seg.UpdateValue(b.sqS, b.sqT, segAdd);
    } 

    SQWORD sqAns = SQWORD_INF;
    for (auto b: vecButton) {
        SEGMENT_VAL_ST valDel(-1);
        seg.UpdateValue(b.sqS, b.sqT, valDel);

        SEGMENT_NODE_ST ans = SEGMENT_NODE_ST::getNodeVal(seg.FindVal(0, MAX_T));
        sqAns = min(ans.stVal.sqVal, sqAns);

        SEGMENT_VAL_ST valAdd(1);
        seg.UpdateValue(b.sqS, b.sqT, valAdd);
    }
