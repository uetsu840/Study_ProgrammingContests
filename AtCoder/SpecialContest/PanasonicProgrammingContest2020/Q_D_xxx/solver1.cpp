...
vector<string> vstrAns;


static void addChar(string s, SDWORD lMax, SDWORD lLength, SDWORD lN)
{
//    printf("[%s]\n", s.c_str());
    for (SDWORD lIdx = 0; lIdx <= lMax + 1; lIdx++) {
        string add = " ";
        add[0] = (char)('a' + lIdx);
        if (lLength + 1 == lN) {
//            printf("---> [%s]\n", (s+add).c_str());

            vstrAns.emplace_back(s + add);
        } else {
            addChar(s + add, max(lMax, lIdx), lLength + 1, lN);
        }
    }
}
