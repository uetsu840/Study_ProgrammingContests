...
#define NODE_GOAL           (1000)
#define INHIBITED_EDGE      (100100100)
...
int main(void)
{
    ...
    /* 1000: goal */
    /* 1-150: station */
    for (SQWORD sqStation = 1; sqStation <= sqN; sqStation++) {
        SQWORD sqP = inputSQWORD();
        SQWORD sqEdgeVal = COST_OFS - sqP;
        /* 止まる */
        stFlow.addEdge(0, sqStation, sqEdgeVal, DUMMY_COST);

        /* 止まらない */
        stFlow.addEdge(sqStation, NODE_GOAL, COST_OFS, DUMMY_COST);
    }
    ...
    for (SQWORD sqIdx = 0; sqIdx < sqM; sqIdx++) {
        ...
        stFlow.addEdge(sqB, sqA, INHIBITED_EDGE, DUMMY_COST);
    }

    SQWORD sqMaxF = stFlow.MaxFlow(0, NODE_GOAL);
    SQWORD sqAns = sqN * COST_OFS - sqMaxF;
