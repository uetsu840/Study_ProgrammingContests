...
int main(void)
{
    ...
    /* 1回目のDP */
    vector<SQWORD> vsqFVisitNum(sqCompNodeNum + 1, 0);
    vector<SQWORD> vsqMaxPath(sqCompNodeNum + 1, -1);
    for (SQWORD sqNode = 1; sqNode <= sqCompNodeNum; sqNode++) {
        if (0 == vvstCompEdge[sqNode].size()) {
            vsqFVisitNum[sqNode] = vvCompNodes[sqNode].size();
        }
    }
    for (auto it = vsqSccTopological.rbegin(); 
            it != vsqSccTopological.rend();
            ++it) {
        for (auto e: vvstCompRevEdge[*it]) {
            SQWORD sqNextNode = e.sqTo;
            SQWORD sqNextVal = vsqFVisitNum[*it] + vvCompNodes[sqNextNode].size();
            if (vsqFVisitNum[sqNextNode] < sqNextVal) {
                vsqFVisitNum[sqNextNode] = sqNextVal;
                vsqMaxPath[sqNextNode] = *it;
            } 
        }
    }

    /* 貪欲に色が塗れる辺をたどる */
    vector<bool> vbMaxAttrPath1(sqCompNodeNum + 1, false);
    {
        SQWORD sqCurNode = max_element(vsqFVisitNum.begin(), vsqFVisitNum.end()) - vsqFVisitNum.begin();
        while(sqCurNode != -1) {
//            printf("=== %lld\n", sqCurNode);
            vbMaxAttrPath1[sqCurNode] = true;
            sqCurNode = vsqMaxPath[sqCurNode];
        }    
    }

    /* 2回目のDP */
    vector<SQWORD> vsqSVisitNum(sqCompNodeNum + 1, 0);
    for (SQWORD sqNode = 1; sqNode <= sqCompNodeNum; sqNode++) {
        if ((0 == vvstCompEdge[sqNode].size()) && (!vbMaxAttrPath1[sqNode])) {
            vsqSVisitNum[sqNode] = vvCompNodes[sqNode].size();
        }
    }
    for (auto it = vsqSccTopological.rbegin(); 
            it != vsqSccTopological.rend();
        ...
        for (auto e: vvstCompRevEdge[*it]) {
            SQWORD sqNextNode = e.sqTo;
            SQWORD sqUpdateVal = vsqSVisitNum[*it];
            if (!vbMaxAttrPath1[sqNextNode]) {
                sqUpdateVal += vvCompNodes[sqNextNode].size();
            }
            if (vsqSVisitNum[sqNextNode] < sqUpdateVal) {
                vsqSVisitNum[sqNextNode] = sqUpdateVal;
            } 
        }
    }

#if 1
    for (auto f: vsqFVisitNum) {
        printf("%lld ", f);
    }
    printf("\n");
    for (auto s: vsqSVisitNum) {
        printf("%lld ", s);
    }
    printf("\n");
#endif

    SQWORD sqAns = *(max_element(vsqFVisitNum.begin(), vsqFVisitNum.end()))
                    + *(max_element(vsqSVisitNum.begin(), vsqSVisitNum.end()));
    printf("%lld\n", sqAns);
