...
int main(void)
{
    ...
    /**
     *  dp[i][j]  長さが i で、末尾8文字の値が j の組み合わせの数
     *  dp[i + |w|][j << |w| | w] = dp[i][j] for each w
     */
    static SQWORD s_asqDpTbl[L_MAX + 1 + W_MAX][DPTBL_W_SIZE];
    s_asqDpTbl[0][0] = 1;
    ...
    for (SQWORD sqIdxL = 0; sqIdxL < sqL; sqIdxL++) {
        for (SQWORD sqTailVal = 0; sqTailVal <= sqTailValMax; sqTailVal++) {
            for (auto w: vecstW) {
                SQWORD sqNewTail = ((sqTailVal << w.sqLen) + w.sqVal) & (0xFF);
                s_asqDpTbl[sqIdxL + w.sqLen][sqNewTail] += s_asqDpTbl[sqIdxL][sqTailVal];
            }
        }
    }

#if 0
    for (SQWORD sqIdxL = 0; sqIdxL <= 5; sqIdxL++) {
        for (SQWORD sqTailVal = 0; sqTailVal < 32; sqTailVal++) {
            printf("%lld ", s_asqDpTbl[sqIdxL][sqTailVal]);
        }
        printf("\n");
    }
#endif    
    ...
    SQWORD sqAns = 0;
    for (SQWORD sqTailVal = 0; sqTailVal <= sqTailValMax; sqTailVal++) {
        sqAns += s_asqDpTbl[sqL][sqTailVal];
    }
