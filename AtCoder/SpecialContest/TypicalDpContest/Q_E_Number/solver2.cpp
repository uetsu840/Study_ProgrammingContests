...
//        printf("---------------[%d]-[%d]\n", lDigitIdx, vDigits[lDigitIdx]);
        ...
        for (SQWORD sqModIdxCur = 0; sqModIdxCur < lInputD; sqModIdxCur++) {
            ...
            for (SQWORD sqMod = 0; sqMod < lInputD; sqMod++) {
                ...
                asqDpTblNext[sqNextIdx] += asqDpTbl[sqModIdxCur] * vsqQuotTbl[sqMod];
                asqDpTblNext[sqNextIdx] %= DIV_MOD;
            }
        }

#if 0
        /* debug */
        for (SDWORD lIdx = 0; lIdx < lInputD; lIdx++) {
            printf("A  %d:%lld\n", lIdx, asqDpTblNext[lIdx]);
        }
#endif
        ...
        for (SDWORD lNum = 0; lNum < lDigitUpper; lNum++) {
            asqDpTblNext[(lStartDigit + lNum) % lInputD]++;
        }
