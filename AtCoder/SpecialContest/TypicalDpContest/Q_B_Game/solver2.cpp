...
int main()
{
    ...
    auto itCurA = vsqTipsA.begin();
    auto itCurB = vsqTipsB.begin();
    

    SQWORD sqAns = 0;
    for (SDWORD lTurn = 0; lTurn < sqInput_A + sqInput_B; lTurn++) {
        SQWORD sqCurScore;

        if (itCurA == vsqTipsA.end()) {
            /* only B */
            sqCurScore = *itCurB;
            itCurB++;
        } else if (itCurB == vsqTipsB.end()) {
            /* only A */
            sqCurScore = *itCurA;
            itCurA++;
        } else {
            /* A and B */
            SQWORD sqScoreA = *itCurA - max(getNextScore(vsqTipsA, itCurA), *itCurB);
            SQWORD sqScoreB = *itCurB - max(getNextScore(vsqTipsB, itCurB), *itCurA);
                
            if (sqScoreA == sqScoreB) {
                if (*itCurB <= *itCurA) {
                    sqCurScore = *itCurA;
                    itCurA++;
                } else {
