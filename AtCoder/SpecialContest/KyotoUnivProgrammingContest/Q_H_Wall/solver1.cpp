...
#define PNT_NO_START    (100001)
#define PNT_NO_TARGET   (100002)
static SQWORD s_asqQtyA[MAX_POSITION];
static SQWORD s_asqQtyB[MAX_POSITION];
...
int main()
{
    ...
    for (DWORD dwIdx = 0; dwIdx < dwInput_N; dwIdx++) {
        s_asqQtyA[dwIdx] = inputSQWORD();
    }
    for (DWORD dwIdx = 0; dwIdx < dwInput_N; dwIdx++) {
        s_asqQtyB[dwIdx] = inputSQWORD();
    }
