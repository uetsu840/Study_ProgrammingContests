...
static ST_GRAPH_EDGE s_astGraphEdgeR[MAX_EDGE_NUM];
static ST_GRAPH_EDGE s_astGraphEdgeB[MAX_EDGE_NUM];

int main()
{
    DWORD dwInput_N, dwInput_A, dwInput_B;
    ...
    /* Red Vertex List */
    #define CAPACITY_RED    (10000)
    #define CAPACITY_BLUE   (1)

    for (DWORD dwIdx = 0; dwIdx < dwInput_A; dwIdx++) {
        s_astGraphEdgeR[dwIdx].lFrom = inputSDWORD();
        s_astGraphEdgeR[dwIdx].lTo   = inputSDWORD();
    }
    for (DWORD dwIdx = 0;dwIdx < dwInput_B; dwIdx++) {
        s_astGraphEdgeB[dwIdx].lFrom = inputSDWORD();
        s_astGraphEdgeB[dwIdx].lTo   = inputSDWORD();
    }
    
    /* Construct G' */
    Flow Gd;
    for (DWORD dwIdx = 0; dwIdx < dwInput_A; dwIdx++) {
        Gd.addEdge(s_astGraphEdgeR[dwIdx].lFrom, s_astGraphEdgeR[dwIdx].lTo, CAPACITY_RED, DUMMY_COST);   
    }
    for (DWORD dwIdx = 0; dwIdx < dwInput_B; dwIdx++) {
        Gd.addEdge(s_astGraphEdgeB[dwIdx].lFrom, s_astGraphEdgeB[dwIdx].lTo, CAPACITY_BLUE, DUMMY_COST);
    }
    DWORD dwMaxFlowGd = Gd.MaxFlow(1, dwInput_N);

    printf("%d\n", dwInput_A + dwInput_B - (dwMaxFlowGd % CAPACITY_RED));
