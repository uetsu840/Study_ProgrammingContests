...
int main(void)
{
    ...
    /* 0-indexed */
    vector<SQWORD> vsqCost(sqN, MAX_COST);
    vsqCost[0] = 0;
    for (SQWORD sqStep = 0; sqStep < sqN; sqStep++) {
        for (SQWORD sqJump = 1; sqJump <= sqK; sqJump++) {
            if (sqStep + sqJump < sqN) {
                vsqCost[sqStep + sqJump] = min(vsqCost[sqStep + sqJump], 
                                            vsqCost[sqStep] + ABS(vsqH[sqStep] - vsqH[sqStep + sqJump]));
            }
        }
    }
