...
int main(void)
{
    ...
    for (SQWORD sqNum = 1; sqNum <= sqN; sqNum++) {
        ...
        solverFlow.addEdge(PNT_NO_START, sqNum, FLOW_OFFSET - sqA, DUMMY_COST);
        solverFlow.addEdge(sqNum, PNT_NO_TARGET, FLOW_OFFSET, DUMMY_COST);

        for (SQWORD sqMul = 2; sqNum * sqMul <= sqN; sqMul++) {
            solverFlow.addEdge(sqNum, sqNum * sqMul, CAPACITY_INF, DUMMY_COST);
        }
    }

    SQWORD sqMaxFlow = solverFlow.MaxFlow(PNT_NO_START, PNT_NO_TARGET);

    printf("%lld\n", FLOW_OFFSET * sqN - sqMaxFlow);
