...
static bool visitNode(SDWORD lNode, vector<SDWORD> vPw, vector<SDWORD> vPb)
{
    printf("%d\n", lNode);

    if (0 == vvChilds[lNode].size()) {
        vPw[vlPnts[lNode]] = 1; 
        vPb[vlPnts[lNode]] = 1;
        return true;
    }

    vector<SDWORD> vlPntsWhite(MAX_PNT+1, 0);
    vector<SDWORD> vlPntsBlack(MAX_PNT+1, 0);

    for (auto it: vvChilds[lNode]) {
        if (!visitNode(it, vlPntsWhite, vlPntsBlack)) {
            return false;
        }
    }
...
int main()
{
    ...
    vector<SDWORD> vlPntsWhite(MAX_PNT+1, 0);
    vector<SDWORD> vlPntsBlack(MAX_PNT+1, 0);
    visitNode(1, vlPntsWhite, vlPntsBlack);
