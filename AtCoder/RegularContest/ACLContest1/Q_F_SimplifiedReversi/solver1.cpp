...
int main(void)
{
    ...
    vector<SEGMENT_NODE_ST> segNodesRow(sqN, sqN - 1);
    vector<SEGMENT_NODE_ST> segNodesCol(sqN, sqN - 1);

    LazySegmentTree segRow(segNodesRow);
    LazySegmentTree segCol(segNodesCol);

    segRow.UpdateValue(sqN - 1, sqN, 0);
    segCol.UpdateValue(sqN - 1, sqN, 0);
    
    SQWORD sqAns = (sqN - 2) * (sqN - 2);
    for (SQWORD sqQueryIdx = 0; sqQueryIdx < sqQ; sqQueryIdx++) {
        SQWORD sqQueryType = inputSQWORD();
        SQWORD sqX = inputSQWORD() - 1;
        if (1 == sqQueryType) {
            /* 1 (1, x) に白い石を置く */
            SQWORD sqStoneRow = segRow.FindVal(sqX, sqX + 1).stVal.sqVal;
            segCol.UpdateValue(0, sqStoneRow + 1, sqX);
            sqAns -= (sqStoneRow - 1);
        } else {
            /* 2 (x, 1) に黒い石を置く */
            SQWORD sqStoneCol = segCol.FindVal(sqX, sqX + 1).stVal.sqVal;
            segRow.UpdateValue(0, sqStoneCol + 1, sqX);
            sqAns = (sqStoneCol - 1);
        }
    }
    printf("%lld\n", sqAns);
