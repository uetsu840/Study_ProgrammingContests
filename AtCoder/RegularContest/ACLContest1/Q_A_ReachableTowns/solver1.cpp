...
int main(void)
{
    ...
    vector<POINT> vstCities;

    for (SQWORD sqIdx = 0; sqIdx < sqN; sqIdx++) {
        SQWORD sqX = inputSQWORD();
        SQWORD sqY = inputSQWORD();
        vstCities.emplace_back(sqX, sqY);
    }

    sort(vstCities.begin(), vstCities.end());

    BinaryIndexedTree<SQWORD> bitLower(sqN);
    BinaryIndexedTree<SQWORD> bitUpper(sqN);

    vector<SQWORD> vsqLower(sqN);
    vector<SQWORD> vsqUpper(sqN);

    for (SQWORD sqCityIdx = 0; sqCityIdx < vstCities.size(); sqCityIdx++) {
        const POINT &cityPos = vstCities[sqCityIdx];
        vsqLower[sqCityIdx] = bitLower.Sum(cityPos.sqY);
        bitLower.Add(cityPos.sqY, 1);
    }
    for (SQWORD sqCityIdx = vstCities.size() - 1; 0 <= sqCityIdx; sqCityIdx--) {
        const POINT &cityPos = vstCities[sqCityIdx];
        vsqUpper[sqCityIdx] = (sqN - 1 - sqCityIdx) - bitUpper.Sum(cityPos.sqY);
        bitUpper.Add(cityPos.sqY, 1);
    }

    for (SQWORD sqIdx = 0; sqIdx < vstCities.size(); sqIdx++) {
        printf("%lld\n", vsqLower[sqIdx] + vsqUpper[sqIdx] + 1);
    }
