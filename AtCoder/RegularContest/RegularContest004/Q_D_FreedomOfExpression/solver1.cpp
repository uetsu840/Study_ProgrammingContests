 ...
 *   分割数を求める
 */
#define DPTBL_SIZE  (100001)
static SQWORD getDivNum(SQWORD sqSum, SQWORD sqDiv)
{
    SQWORD sqDivNum = combMod(sqSum + sqDiv - 1, sqDiv - 1);

//    printf("g: %lld %lld -> %lld\n", sqSum, sqDiv, sqDivNum); 
...
int main(void)
{
    ...
    /* 0は入力されない (1 <= |N|) */
    calcPrimeFactorication(abs(lInput_N), vlPrimes);

    SQWORD sqAns = 1;
    for (auto it:vlPrimes) {
        sqAns = (sqAns * getDivNum((SQWORD)it, lInput_M)) % ANS_MOD;
    }

    /* 符号を考慮 */
    for (SDWORD lIdx = lInput_M - 1; 0 < lIdx; lIdx--) {
        sqAns = mulMod(sqAns, 2);
    }
    printf("%lld\n", sqAns);
