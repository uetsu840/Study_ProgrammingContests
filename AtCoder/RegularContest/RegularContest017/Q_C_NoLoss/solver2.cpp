...
//    printf("%d %d %d [%d]\n", dwIdx, dwRest, dwObjNum, s_adwRevCumSum[dwIdx]);
    if (dwIdx == dwObjNum - 1) {
...
//            printf("x\n");
        ...
        if (s_adwRevCumSum[dwIdx] < dwRest) {
            return 0;
        }
...
int main()
{
  ...
  //      printf("-- %d\n", s_adwSize[dwIdx]);
    ...
    DWORD dwSum = 0;
    for (DWORD dwIdx = dwInput_N; 0 < dwIdx; dwIdx--) {
        dwSum += s_adwSize[dwIdx - 1];
        s_adwRevCumSum[dwIdx - 1] = dwSum;
    }
