...
static inline SQWORD DIV_UP(SQWORD a, SQWORD b) { return (a + (b-(SQWORD)1)) / b; };
    ...
    const PROBLEM_ONE *pstProb)
{
    ...
    if (MAX_SQWORD / sqJudge < pstProb->sqY) {
        return false;
    }

    SQWORD sqUpper = (pstProb->sqR - sqJudge) / (pstProb->sqX - (SQWORD)1);
    SQWORD sqLower = DIV_UP(sqJudge * pstProb->sqY - pstProb->sqB, pstProb->sqY - (SQWORD)1);
...
{
    PROBLEM_ONE stProb;
    stProb.sqR = inputSQWORD();
    stProb.sqB = inputSQWORD();
