...
static void addDpTbl(SQWORD &sqDp, SQWORD sqUpd)
{
    if (0 < sqUpd) {
        sqDp = min(sqDp + (SQWORD)1, (SQWORD)2);            
    }
}

static void subDpTbl(SQWORD &sqDp, SQWORD sqUpd)
{
    if (0 < sqUpd) {
        sqDp = max(sqDp - (SQWORD)1, (SQWORD)0);
    }
}
...
static SQWORD countNeedCards(
    ...
    SQWORD sqK)
{
    /* dp(i)[j]  i枚目までを考慮したとき、合計がjになるような通り数(1: 直前に1通り、 2:直前に2通り) */
    ...
    for (SQWORD sqCardIdx = 0; sqCardIdx < vsqA.size(); sqCardIdx++) {
        for (SQWORD sqDpIdx = vsqDp.size() - 1; 0 <= sqDpIdx; sqDpIdx--) {
            ...
            if (sqNextIdx < sqK) {
                addDpTbl(vsqDp[sqNextIdx], vsqDp[sqDpIdx]);
            }
        }
    }
#if 0
    ...
    */
    ...
    for (auto a: vsqA) {
        ...
        for (SQWORD sqDp2Idx = 0; sqDp2Idx < vsqDp2.size(); sqDp2Idx++) {
            vsqDp2[sqDp2Idx] = (0 < vsqDp[sqDp2Idx]) ? 1 : 0;
            if (a <= sqDp2Idx) {
                subDpTbl(vsqDp2[sqDp2Idx], vsqDp2[sqDp2Idx - a]);
            }
        }
//        printVector(vsqDp2);
        ...
        if (vsqDp2.size() < a) {
            ...
            for (SQWORD sqDp2Idx = vsqDp2.size() - a; sqDp2Idx < vsqDp2.size(); sqDp2Idx++) {
...
static void solve(SQWORD sqN, SQWORD sqK, const vector<SQWORD> &vsqA)
{
