...
#define MAX_CHECKED_GIRLS   (100)
#define MAX_RELATION_NUM    (5000)
#define DUMMY_COST          (0)
typedef struct {
    SDWORD lA;
    SDWORD lB;
...
static SDWORD       s_alCheckedGirls[MAX_CHECKED_GIRLS];
static ST_RELATION  s_astRelations[MAX_RELATION_NUM];

#define PNT_NO_OFFSET_IN    (0)     /* 1-100 */
#define PNT_NO_OFFSET_OUT   (100)   /* 101-200 */
#define PNT_NO_TARGET       (201)

int main()
{
    DWORD dwInput_N, dwInput_G, dwInput_E;

    dwInput_N = inputSDWORD();
    dwInput_G = inputSDWORD();
    dwInput_E = inputSDWORD();

    for (DWORD dwIdx = 0; dwIdx < dwInput_G; dwIdx++) {
        s_alCheckedGirls[dwIdx] = inputSDWORD();
    }
    for (DWORD dwIdx = 0; dwIdx < dwInput_E; dwIdx++) {
        s_astRelations[dwIdx].lA = inputSDWORD();
        s_astRelations[dwIdx].lB   = inputSDWORD();
    }

    Flow solverFlow;
    for (DWORD dwIdx = 0; dwIdx < dwInput_E; dwIdx++) {
        solverFlow.addEdge(s_astRelations[dwIdx].lA + PNT_NO_OFFSET_OUT, 
                            s_astRelations[dwIdx].lB + PNT_NO_OFFSET_IN,
                            1, DUMMY_COST);
        solverFlow.addEdge(s_astRelations[dwIdx].lB + PNT_NO_OFFSET_OUT,
                            s_astRelations[dwIdx].lA + PNT_NO_OFFSET_IN,
                            1, DUMMY_COST); 
    }
    for (DWORD dwIdx = 0; dwIdx < dwInput_N; dwIdx++) {
        solverFlow.addEdge(dwIdx + PNT_NO_OFFSET_IN,
                            dwIdx + PNT_NO_OFFSET_OUT,
                            1, DUMMY_COST);
    }

    for (DWORD dwIdx = 0; dwIdx < dwInput_G; dwIdx++) {
        solverFlow.addEdge(s_alCheckedGirls[dwIdx] + PNT_NO_OFFSET_OUT,
                            PNT_NO_TARGET,
                            1, DUMMY_COST);
    }
    DWORD dwMaxFlow = solverFlow.MaxFlow(0 + PNT_NO_OFFSET_OUT, PNT_NO_TARGET);
    printf("%d\n", dwMaxFlow);
