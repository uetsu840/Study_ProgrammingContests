...
#define START_PNT_NO    (200)
#define TARGET_PNT_NO   (201)
#define MAX_PNT_NUM     (100)           /* 0-99 */
#define DUMMY_COST      (0)
#define BLUE_PNT_OFFSET (MAX_PNT_NUM)   /* 100-199 */
typedef struct {
    SDWORD lX;
    SDWORD lY;
} ST_POINT_POS;

static ST_POINT_POS s_astPosR[MAX_PNT_NUM];
static ST_POINT_POS s_astPosB[MAX_PNT_NUM];
...
int main()
{
    ...
    for (DWORD dwIdx = 0; dwIdx < dwInput_N; dwIdx++) {
        s_astPosR[dwIdx].lX = inputSDWORD();
        s_astPosR[dwIdx].lY = inputSDWORD();
    }
    for (DWORD dwIdx = 0;dwIdx < dwInput_N; dwIdx++) {
        s_astPosB[dwIdx].lX = inputSDWORD();
        s_astPosB[dwIdx].lY = inputSDWORD();
    }
    
    /* Construct G' */
    Flow solverFlow;
    for (DWORD dwIdxR = 0; dwIdxR < dwInput_N; dwIdxR++) {
        for (DWORD dwIdxB = 0; dwIdxB < dwInput_N; dwIdxB++) {
            if ((s_astPosR[dwIdxR].lX < s_astPosB[dwIdxB].lX) 
                && (s_astPosR[dwIdxR].lY < s_astPosB[dwIdxB].lY)) {
                solverFlow.addEdge(dwIdxR, dwIdxB + BLUE_PNT_OFFSET, 1, DUMMY_COST);
            }
        }
    }
    for (DWORD dwIdxR = 0; dwIdxR < dwInput_N; dwIdxR++) {
        solverFlow.addEdge(START_PNT_NO, dwIdxR, 1, DUMMY_COST);
    }
    for (DWORD dwIdxB = 0; dwIdxB < dwInput_N; dwIdxB++) {
        solverFlow.addEdge(dwIdxB + BLUE_PNT_OFFSET, TARGET_PNT_NO, 1, DUMMY_COST);
    }
    DWORD dwMaxFlow = solverFlow.MaxFlow(START_PNT_NO, TARGET_PNT_NO);
