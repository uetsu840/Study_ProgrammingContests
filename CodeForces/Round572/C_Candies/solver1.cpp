...
int main(void)
{
    ...
    for (auto num: vecsqInput_a) {
        sqCumSum += num;
        vecsqCumSum.emplace_back(sqCumSum);
    }

    SQWORD sqNumQuery = inputSQWORD();
    for (SQWORD sqQueryIdx = 0; sqQueryIdx < sqNumQuery; sqQueryIdx++) {
        SQWORD sqInput_l = inputSQWORD();
        SQWORD sqInput_r = inputSQWORD();

        printf("%lld\n", (vecsqCumSum[sqInput_r] - vecsqCumSum[sqInput_l - 1]) / 10);
    }
